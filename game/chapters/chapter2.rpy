...
show u greeting offended at center with Dissolve(0.5)
...
d_t pose2 pupupu "Я склонился в поклоне и сложил руки лодочкой в извиняющемся жесте."

d "Прости! За что бы то ни было!"

show u closed neutral right
...
show u greeting offended at move_step(offset=50)
...
u touched "В прошлый раз мы с папой так и не разобрали гараж, и собирались заняться этим сегодня. Поможешь?"

d pose1 happy "Легко!"

show u open thinking right
...
show u open thinking right at c_left
show bg_near_home 
with Dissolve(1)
...
show b_1 at move_on_scene(xalign=0.3)
...
show b_1 right at move_on_scene(xalign=0.5)
show u at move_on_scene(xalign=0.8)
...
show b_1 left at Transform(xalign=1.1, yalign=1.0) with Dissolve(0.5)

d_t "Вынести мусор? Выкинул 3 моментально собранных мешка в окошко под потолком."

show b_1 at fear
$ renpy.pause(0.5)
show b_1 at move_on_scene(tim1=0.5, xalign=1.5)
...
show b_1 right at enter_scene(from_left=True, xalign=0.2)
...
show u closed cute right at enter_scene(from_left=True, xalign=0) with Dissolve(0.5)
...
hide guitar_solo with Dissolve(1)
...
show u open thinking at move_on_scene(xalign=0.6)
$ renpy.pause(2.0)
show u left closed cute
...
show b_1 left at Transform(xalign=0.6, yalign=1.0)
show u left open thinking at Transform(xalign=0.8, yalign=1.0)
show bg_near_home with Dissolve(1)
$ renpy.pause(0.5)
show u neutral at move_on_scene(xalign=0.4)

d_t "Отец уже начал закрывать калитку, как в неё просочилась Юми. "

show u closed touched with Dissolve(0.5)
show b_1 right at move_on_scene(xalign=1.5)

d_t "Она улыбнулась, шепнула \"Спасибо!\" и поцеловала меня в щеку. "

hide b_1

d_t relief "Я понял, что не успел загрузить пособие по пикапу от Алекса Лесли, поэтому не знал, как отреагировать. Пришлось просто улыбнуться ей."

d_t "Всё, что я смог, это прошептать ей в ответ:"

show u cute

d cunning "Пойдём завтра в 12 гулять в парк?"

show u touched
$ renpy.pause(0.5)
show u greeting smile 
$ renpy.pause(0.5)
show u open cute right at move_on_scene(xalign=1.5)
...
show t_f ear surprised at Transform(xalign=0.3, yalign=1.0) with Dissolve(0.5)
...
show t_f at Transform(xalign=0.3, yalign=1.5), fear with Dissolve(0.5)
...
show t_f thinking asharashen at Transform(xalign=0.3, yalign=1.0), fear with Dissolve(0.2)
...
d_f sad "Господин Тайда, вам нужно остыть и подумать. А я пока помогу вашей маме."
...
t_t hz cry_sad "Драка закончилась, не успев начаться. Конечно, же моим поражением и безоговорочной капитуляцией. Я получил мощного пинка под зад железной ногой. "
...
t_t think thinking_hard "Что же я наделал..."
...
t ear surprised "Друзья, проиошла беда. Дело в Дзндзо. Приходите срочно пожалуста."
...
t_t thinking tired naked "Спустя какое-то время я услышал стук в окно. "

t_t "На улице темнело. За стеклом были видны три силуэта."

t_t "Кацуми, Ден и Хикару."

show k pose1 worried left at trio_left
show den idle nervous summer left at trio_center
show h idle surprised left at trio_right
with Dissolve(1)
...
t think thinking_hard "Н-нет... С ним всё в порядке. Он функционирует, и даже слишком хорошо для робота."
...
show d_f pose2 neutral right at Transform(xalign=0.4, yalign=1.0)
