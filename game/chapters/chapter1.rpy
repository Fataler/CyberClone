label chapter1:
    ...
    #t neutral
    t_t neutral "Немного замешкавшись со сбором вещей, я одним из последних направился к выходу, но..."
...
#    $ renpy.music.set_volume(1.0, delay=0.5, channel="music")
    ...
    k "Хэй, Тайда!"
    window hide
    show screen profile_katsumi with Dissolve(0.5)
    pause 10
    ...
    # экран den
    window hide
    show screen profile_den with Dissolve(0.5)
    pause 10
    ...
    t_t "Дорога приключений привела нас, правда, не в грандиозный поход к подножию скалы Хокаге и не в жерло Роковой Горы, а во всё тот же кружок робототехники. "
    ...
    window hide
    show screen profile_umi with Dissolve(0.5)
    pause 10
    ...
    t_t "Это были попытки запрограммировать робота так, чтобы он плавно набирал паштет из ёмкости и вёл его по ломтику хлеба..."

    t_t "...а не пытался швырнуть в нас банкой или пырнуть под ребро."
    ...
    #t shy happy
    t_t happy "Помимо того, что она очень милая и добрая, крайне ответственная, хозяйственная и понимающая, она умеет здорово играть на музыкальных инструментах и..."
    t_t crazy "В аркадные игровые автоматы."
    ...
    #t depressed
    t_t depressed "В последний раз, когда мы проиграли там карманные деньги с обедов за 2 недели, её отец строго-настрого запретил ей приближаться к залу."
    ...
    #t neutral
    t_t neutral "Я уже собрался попрощаться с подругой, как она задала вопрос:" 
    ...
    t_t ear_school fear "Я не очень хотел в такой классный день разбирать грязные шмотки её отца в гараже и выслушивать очередные \"удивительные\" истории о его рокерском прошлом."
    ...
    t_t hz wtf "А ничо тот факт, что в сутках нет столько часов, вы вообще в курсе?!"

    #t very angry
    t_t cry_sad "Я так ничего не добьюсь! Они понимают, что я строю карьеру? Настоящую, цифровую!"

    #t sad
    t_t ear_school cry_angry "Для этого нужно вкладывать усилия, время, пот, кровь, слезы и донат! "
